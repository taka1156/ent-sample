...
func (uq *UserQuery) QueryCategory() *CategoryQuery {
...
func (uq *UserQuery) QueryGoal() *GoalQuery {
	query := &GoalQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withCategory: uq.withCategory.Clone(),
		withRole:     uq.withRole.Clone(),
		withTeam:     uq.withTeam.Clone(),
		withGoal:     uq.withGoal.Clone(),
...
func (uq *UserQuery) WithCategory(opts ...func(*CategoryQuery)) *UserQuery {
	query := &CategoryQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withCategory = query
	return uq
}
...
func (uq *UserQuery) WithGoal(opts ...func(*GoalQuery)) *UserQuery {
	query := &GoalQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withCategory; query != nil {
		if err := uq.loadCategory(ctx, query, nodes,
			func(n *User) { n.Edges.Category = []*Category{} },
			func(n *User, e *Category) { n.Edges.Category = append(n.Edges.Category, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := uq.withGoal; query != nil {
		if err := uq.loadGoal(ctx, query, nodes, nil,
			func(n *User, e *Goal) { n.Edges.Goal = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadCategory(ctx context.Context, query *CategoryQuery, nodes []*User, init func(*User), assign func(*User, *Category)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.CategoryTable)
		s.Join(joinT).On(s.C(category.FieldID), joinT.C(user.CategoryPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.CategoryPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.CategoryPrimaryKey[0]))
...
func (uq *UserQuery) loadGoal(ctx context.Context, query *GoalQuery, nodes []*User, init func(*User), assign func(*User, *Goal)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*User)
	for i := range nodes {
		fk := nodes[i].GoalID
