...
func (cq *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := cq.withUser; query != nil {
		if err := cq.loadUser(ctx, query, nodes,
			func(n *Category) { n.Edges.User = []*User{} },
			func(n *Category, e *User) { n.Edges.User = append(n.Edges.User, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CategoryQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Category, init func(*Category), assign func(*Category, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(category.UserTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(category.UserPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(category.UserPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(category.UserPrimaryKey[1]))
